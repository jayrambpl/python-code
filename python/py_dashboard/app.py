...
if authenticator_status:
    ...
    if file1 is not None:
        ...
        Input_file1 = "Report_20240110_134318.xlsx"
    ...
    def get_excel_data(Input_file1):
        ...
        df1 = pd.read_excel(io=Input_file1,
                            engine="openpyxl",
                            sheet_name="IoT_Bench",
                            usecols="B:R"
                            )
        df2 = pd.read_excel(io=Input_file1,
                            engine="openpyxl",
                            sheet_name="IoT_Upcoming_Bench",
                            usecols="B:R")
        return df1,df2
    ...
    tower_selected= st.sidebar.multiselect("Select Tower for Bench Data",
                                options=df1["Skill"].unique(),
                                default=df1["Skill"].unique()
                                )
    ...
    df1_selected = df1.query(
        "Skill==@tower_selected & BandAsPerSR==@band_selected")
    # =============
    ...
    upcoming_tower_selected= st.sidebar.multiselect("Select Tower for Upcoming Bench Data",
                                options=df2["Skill"].unique(),
                                default=df2["Skill"].unique()
                                )
    ...
    df2_selected = df2.query(
        "Skill==@upcoming_tower_selected & BandAsPerSR==@upcoming_band_selected")
    ...
    data1 = df1_selected.groupby(['Skill', 'BandAsPerSR']).size().reset_index(name='Count of band')
    df1_selected = pd.DataFrame(data1)
    data2 = df2_selected.groupby(['Skill', 'BandAsPerSR']).size().reset_index(name='Count of band')
    ...
    with fig1:
        ...
        fig = px.bar(df1_selected, x="Skill", y="Count of band",
    ...
    with fig2:
        ...
        fig = px.bar(df2_selected, x="Skill", y="Count of band",
    ...
    with view3:
        ...
        if sel_row1:
            ...
            skill_value = sel_row1[0]["Skill"]
            name_r = sel_row1[0]["Practitioner Notes ID"]
            name_emp = name_r.split("/")[0]

            st.subheader("Selected Resource Details:", divider='rainbow')
            st.write(f"\nName : {name_emp}")
            st.write(f"JRSS-Primary : {jrss_primary_value}")
            st.write(f"Skill : {skill_value}")
            utilization_13 = 0.0
            st.write(f"13 wk Utilization : {utilization_13:.1%}")
            st.balloons()
        else:
    ...
    with view4:
        ...
        with st.expander("Tower wise Bench Data"):
            ...
            if sel_row2:
                ...
                skill_value1 = sel_row2[0]["Skill"]
                name_r1 = sel_row2[0]["Practitioner Notes ID"]
                name_emp1 = name_r1.split("/")[0]
                st.subheader("Selected Resource Details:", divider='rainbow')
